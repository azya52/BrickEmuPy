...
RAM_SIZE = 96
GRAM_SIZE = 16
FUULL_VRAM = tuple([255] * GRAM_SIZE)
...
class MSM50XX():
    def __init__(self, mask, clock):
        ...
        self._sound = MSM50XXsound(clock)
        ...
        self._execute = (
            ...
            *([MSM50XX._dummy] * 32),
            *([MSM50XX._dspf_digit_ap] * 32),      #00 110P DDDD AAAA
            *([MSM50XX._dummy] * 32),
    ...
    def _jmp_ap(self, opcode):
        #00 0001 1101 AAAA
        self._PC += self._RAM[(self._PAGE << 4) + (opcode & 0xF)]
    ...
    def _jmpio_ap(self, opcode):
        #00 0011 1101 AAAA
        self._PC += self._RAM[(self._PAGE << 4) + (opcode & 0xF)] & 0x7

    def _cmp_acc_ap(self, opcode):
        #00 0011 1110 AAAA
        self._ZF = self._RAM[(self._PAGE << 4) + (opcode & 0xF)] == self._ACC
    ...
    def _dspf_digit_ap(self, opcode):
        ...
        if (opcode & 0x0100):
            a += (self._PAGE << 4)
        d = (opcode >> 4) & 0xF
        offset = (self._PC & 0xF00) | ((self._FMT & 0x6) << 3) | 0x0C0 | self._RAM[a]
    ...
    def _switch_ap(self, opcode):
        #11 0101 0001 AAAA
        self._ACC = self._RAM[(self._PAGE << 4) + (opcode & 0xF)] = self._PS
        self._ZF = not self._ACC

    def _kswitch_ap(self, opcode):
        #11 0101 0010 AAAA
        self._ACC = self._RAM[(self._PAGE << 4) + (opcode & 0xF)] = self._PK
        self._ZF = not self._ACC

    def _intmode_ap(self, opcode):
        #11 0101 0100 AAAA
        self._ACC = self._RAM[(self._PAGE << 4) + (opcode & 0xF)] = self._INT
        self._INT = 0 #?
        self._ZF = not self._ACC

    def _rate_ap(self, opcode):
        #11 0101 1001 AAAA
        self._ACC = self._RAM[(self._PAGE << 4) + (opcode & 0xF)] = self._TC
        self._ZF = not self._ACC
    ...
    def _matrix_ap(self, opcode):
        #11 0111 0010 AAAA
        self._PM = self._RAM[(self._PAGE << 4) + (opcode & 0xF)]

    def _format_ap(self, opcode):
        #11 0111 0011 AAAA
        self._FMT = self._RAM[(self._PAGE << 4) + (opcode & 0xF)] 
    ...
    def _mov_acc_ap(self, opcode):
        #11 1101 0000 AAAA
        self._RAM[(self._PAGE << 4) + (opcode & 0xF)] = self._ACC
    ...
    def _mov_ap_acc(self, opcode):
        #11 1111 0000 AAAA
        self._ACC = self._RAM[(self._PAGE << 4) + (opcode & 0xF)]
        self._ZF = not self._ACC
