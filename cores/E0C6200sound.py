    ...
    def clock(self):
        ...
        if (self._envelope_counter > 0):
            ...
            if (self._envelope_counter <= 0):
                ...
                self._tone_generator.play(self._buzzer_freq, False, 1 / (8 - self._envelope_step), self._cycle_counter / self._system_clock)
                self._envelope_counter = self._envelope_cycle

    def set_freq(self, value):
        self._buzzer_freq = self._system_clock / BUZZER_FREQ_DIV[value]
        if (self._sound_on):
            self._tone_generator.play(self._buzzer_freq, False, 0.5, self._cycle_counter / self._system_clock)
